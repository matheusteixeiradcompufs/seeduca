...
class GetMeAPITest(PessoasAPITestBase):
    ...
    def setUp(self) -> None:
        supersetup = super(GetMeAPITest, self).setUp()

        self.get_me = 'pessoas:get_me'

        self.data = {
            'username': self.funcionario.usuario.username,
        }

        self.data2 = {
            'user': self.aluno.usuario.username,
        }
    ...
    def test_if_get_me_returns_bad_request(self):
        self.logar_professor()
        url = reverse(self.get_me)
    ...
    def test_if_get_me_returns_OK_if_superuser(self):
        self.logar_aluno()
